...
//  TestProject
//
//  Created by Reed Carson on 7/2/18.
//  Copyright © 2018 Carsonios. All rights reserved.
//

import UIKit
import AVFoundation
import CardTitleReader
...
class ViewController: UIViewController, CardTitleReaderDelegate {
    //MARK: - Outlets
  ...
  //  var visionOrientation: VisionDetectorImageOrientation = .rightTop
    ...
    let mtgTitleReader = CardTitleReader()
...
//        }
    ...
    // MARK: - TitleReader Delegate Methods
    ...
    func didDetectElement(_ element: TextElement) {
        DispatchQueue.main.async {
            self.addDebugFrameToView(element.frame)
        }
    }
}
...
extension ViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if outputCounter > captureSampleBufferRate {
            mtgTitleReader.processSampleBuffer(sampleBuffer)
            outputCounter = 0
        }
    }
}
